...
public final class StopGameSubCommand extends AdminSubCommand {

    private final ArenaManager arenaManager;
    private final GameManager gameManager;

    public StopGameSubCommand(MessageManager messages, ArenaManager arenaManager, GameManager gameManager) {
        super(messages);
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (!ArenaCommandSupport.requireArenaArgument(sender, args, 1, messages, "game.usage-stop")) {
            return true;
        }

        String id = args[1];
        arenaManager.getArena(id).ifPresentOrElse(
                arena -> sendResult(sender, gameManager.stopGame(arena), arena.displayName()),
    ...
    private void sendResult(CommandSender sender, GameActionResult result, String arenaName) {
        String path = switch (result) {
            case SUCCESS -> "game.stopped";
            case NOT_RUNNING -> "game.not-running";
            case ALREADY_RUNNING, NO_PLAYERS, MISSING_SPAWNS -> "game.not-running";
        };
